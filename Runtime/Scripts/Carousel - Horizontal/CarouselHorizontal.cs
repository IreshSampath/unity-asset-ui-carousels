...
public class CarouselHorizontal : MonoBehaviour, IPointerDownHandler
{
    ...
    void Start()
    {
        ...
        //TOEnable _carouselCards[_currentCardIndex].GetComponent<RectTransform>().DOScale(new Vector2(_cardsMaxScale, _cardsMaxScale), 0);
        //TOEnable _carouselCards[_currentCardIndex].transform.GetChild(0).GetComponent<RectTransform>().DOAnchorPosY(-_cardsGapY, 0);
    ...
    void CreatePaginationDots()
    {
        ...
        foreach (var child in _carouselCards)
        {
            ...
            if (_currentCardIndex == i)
            {
                ...
                //TOEnable dotRectTransform.DOScale(new Vector3(1.5f, 1.5f), 0.25f);
    ...
    void HandleCarousel(int direction)
    {
        ...
        foreach (GameObject card in _carouselCards)
        {
            ...
            //TOEnable cardRectTransform.DOAnchorPos(new Vector2(cardRectTransform.anchoredPosition.x + (_cardsGapX * direction), cardRectTransform.anchoredPosition.y), _transitionTime);
            ...
            if (i == _currentCardIndex)
            {
                ...
                //TOEnable cardRectTransform.GetChild(0).GetComponent<RectTransform>().DOAnchorPosY(-_cardsGapY, _transitionTime);
                //TOEnable cardRectTransform.DOScale(new Vector2(_cardsMaxScale, _cardsMaxScale), 0.25f);
            ...
            else
            {
                //cardRectTransform.DOAnchorPos(new Vector2(cardRectTransform.anchoredPosition.x + (_cardsGapX * direction), cardRectTransform.anchoredPosition.y + _cardsGapY), _transitionTime);
                //TOEnable cardRectTransform.GetChild(0).GetComponent<RectTransform>().DOAnchorPosY(_cardsGapY, _transitionTime);
                //TOEnable cardRectTransform.DOScale(new Vector2(_cardsMinScale, _cardsMinScale), 0.25f);

                _paginationDots[i].GetComponent<Image>().sprite = _paginationDotDefaultImg;
                //TOEnable dotRectTransform.DOScale(new Vector3(1f, 1f), 0.25f);
